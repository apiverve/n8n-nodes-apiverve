        ...
        this.methods = {
            loadOptions: {
                async getAvailableAPIs() {
                    try {
                        const response = await this.helpers.httpRequestWithAuthentication.call(this, 'apiVerveOAuth2Api', { method: 'GET', url: 'https://api.apiverve.com/v1/n8n/apis' });
            ...
            try {
                ...
                if (resource === 'api') {
                    if (operation === 'execute') {
                        const apiId = this.getNodeParameter('apiId', i);
                        const parametersJson = this.getNodeParameter('parameters', i);
                        let params = {};
                        try {
                            ...
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Invalid JSON in parameters');
                        }
                        response = await this.helpers.httpRequestWithAuthentication.call(this, 'apiVerveOAuth2Api', { method: 'GET', url: `https://api.apiverve.com/v1/${apiId}`, qs: params });
                    }
                    else if (operation === 'list') {
                        response = await this.helpers.httpRequestWithAuthentication.call(this, 'apiVerveOAuth2Api', { method: 'GET', url: 'https://api.apiverve.com/v1/n8n/apis' });
                    }
                }
                // ========== JSON Bin Resource ==========
                    ...
                    if (operation === 'list') {
                        response = await this.helpers.httpRequestWithAuthentication.call(this, 'apiVerveOAuth2Api', { method: 'GET', url: `${baseUrl}/list` });
                    }
                    else if (operation === 'get') {
                        const binId = this.getNodeParameter('binId', i);
                        response = await this.helpers.httpRequestWithAuthentication.call(this, 'apiVerveOAuth2Api', { method: 'GET', url: `${baseUrl}/get/${binId}` });
                    }
                    else if (operation === 'update') {
                        const binId = this.getNodeParameter('binId', i);
                        const body = { data: JSON.parse(this.getNodeParameter('binData', i)) };
                        response = await this.helpers.httpRequestWithAuthentication.call(this, 'apiVerveOAuth2Api', { method: 'PUT', url: `${baseUrl}/update/${binId}`, body });
                    }
                }
                // ========== Analytics Resource ==========
                else if (resource === 'analytics') {
                    response = await this.helpers.httpRequestWithAuthentication.call(this, 'apiVerveOAuth2Api', { method: 'GET', url: 'https://api.apiverve.com/v1/n8n/analytics' });
                }
